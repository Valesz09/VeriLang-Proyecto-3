...
// Recorre el AST y genera una representación en cadena del resultado evaluado del programa.
// Luego, lo imprime en la consola.

// VeriLang es un lenguaje de *especificación*, no un lenguaje de propósito general.

// "Ejecutar" un programa significa:
// 1. Imprimir cada variable declarada con su tipo y valor (si es un literal).

// 2. Imprimir cada estructura de datos con sus campos.

// 3. Evaluar e imprimir cada ecuación (lado izquierdo = lado derecho).
// ─────────────────────────────────────────────────────────────────────────────
...
// Ejecuta un archivo .vl: analiza, comprueba el tipo y luego imprime los resultados.void runFile(loc file) {
...
void runSource(str src, loc origin) {
    ...
    // Primero, realiza una comprobación de tipos; informa de los errores, pero continúa imprimiendo una salida parcial.    set[Message] msgs = checkProgram(prog, origin);
    if (!isEmpty(msgs)) {
        println("=== Type errors ===");
